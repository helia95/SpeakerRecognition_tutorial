...
def get_embeddings(use_cuda, filename, model, test_frames):
    ...
    activation = [l2_norm(act, 1) for act in activation]
    activation_th = torch.cat(activation, dim=0)
    return activation_th
